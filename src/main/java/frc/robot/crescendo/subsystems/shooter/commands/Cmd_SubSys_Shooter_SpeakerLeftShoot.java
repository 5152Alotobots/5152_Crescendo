 ...
 */
public class Cmd_SubSys_Shooter_SpeakerLeftShoot extends Command {
    private SubSys_Shooter subSysShooter;
    private boolean atPos = false;

    Timer timer = new Timer();

    public Cmd_SubSys_Shooter_SpeakerLeftShoot(
    ...
    public void initialize() {
        timer.reset();
        timer.start();
        subSysShooter.setShooterArmPosCmd(ShooterArm.ShooterArmSpeakerLeftPos);
    ...
    public void execute() {
        if(!atPos){
            atPos = subSysShooter.setShooterArmPosCmd(ShooterArm.ShooterArmSpeakerLeftPos);
            if(atPos){
                timer.start();
                subSysShooter.shootSpeakerShooterWheels();
            }
